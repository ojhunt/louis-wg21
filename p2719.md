...
0. Harass Erich and Hana - try to steal time on monday
  a) constexpr?
  b) ???
1. Ensure wording is precise around type_identity, note it prevents alias templates -> needs to go to EWG
2. Fix wording around scoping: just say you must have operator new and operator delete in same scope - class declaration with type aware new or delete must have both - don't need both to be type aware
3. Try to fix operator new->operator delete, operator new[]->operator delete[] language for scope restriction
4. Before 11.1 steal lauri's work on how to construct the candidates to replace p10 in expr.delete
5. Verify wording of alignment argument in operator new - 90% it just YOLOs whatever is there
