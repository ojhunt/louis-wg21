...
[As with the changes to [basic.stc.dynamic.allocation]{.sref} the intent of this wording is to permit the first parameter to be a `std::type_identity` specialization as long as we can identify this as a specialization of `std::type_identity` prior to any template instantiation]{.ednote font-family="monospace"}
...
[these points exist, but are being inset and numbered/put into a sub-list]{.ednote font-family="monospace"}
...
[end the added inset + new sublist]{.ednote font-family="monospace"}
...
[This restriction was not discussed in Wroclaw, but we believe is a useful hazard restriction given the lack of any mechanism to otherwise force co-definition of new and delete operators]{.ednote font-family="monospace"}
...
have a type-identity of type `std::type_identity<X>`?]{.ednote font-family="monospace"}
...
[It is unclear if this last constraint needs to be stated explicitly as
it would require a non-standard declaration of std::type_identity which I think would be
UB?]{.ednote font-family="monospace"}
