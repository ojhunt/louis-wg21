...
> A C++ implementation provides access to, and management of, dynamic storage via the global [_allocation functions_ `operator new` and `operator new[]`](allocation){.sub} and [the global _deallocation functions_ `operator delete` and `operator delete[]`]{.rm} [deallocation functions]{.add}.
>
...
[Not sure about this]{.draftnote}

> A destroying operator delete cannot be a function template, because it cannot have a type-identity parameter.

[Maybe? this]{.draftnote}

> A destroying operator delete cannot be type-aware deallocation function, and so cannot be a template function.

[My thinking: the important restriction here is that a destroying delete cannot be type-aware, and so having that at the front seems valuable? also the note reinforces that the lack of support for a type-identity parameter is intentional? But also i clearly don't understand core wording well enough to be confident in anything that i think?]{.draftnote}
...
> - [30.2]{.pnum} Each candidate whose parameter-type-list is not identical to that of the allocation function, ignoring their respective first parameters, is removed from the set of candidates.

[
  "ignoring their respective first parameters"

  this skips only the first parameter, if the type-aware allocation function
  was used, that skips the type-identity, but not the size, so:

```cpp
  struct S {
    void *operator new(std::type_identity<S>, std::size_t, std::align_val_t, float);
    // the theoretically matching operator delete:
    void operator delete(std::type_identity<S>, void*, std::size_t, std::align_val_t, float);
  };
  new (1.5f) S;
```

The allocation function is resolved to `S::operator new(std::type_identity<S>, std::size_t, std::align_val_t, float)`, but when we attempt to resolve the deallocation function we
do that by comparing the arguments parameter list by simply dropping their first parameter.

Now when we compare the remaining parameters of the function we're comparing
`{std::size_t, std::align_val_t, float}` to `{void*, std::size_t, std::align_val_t, float}`

There is an existing problem in the _existing_ language in the context of sized deallocation
functions: because the sized deallocation function has an additional size parameter after
the address parameter, `operator delete(void*, size_t, placement-parameter-types...)` is not a valid deallocation candidate
here for `operator new(size_t, placement-parameter-types...)`, as similar to our case dropping the first parameter results
in a comparison of `{placement-parameter-types...}` to `{size_t, placement-parameter-types...}`
which causes problem if you do want you deallocation type to be able to make use
of the allocation size -- see [P3492].

If the selected allocation function is a type-aware allocation function, then the deallocation
function must be type-aware. Then the comparison would be after dropping the first three
parameters of the allocation function, and dropping the first four parameters of the
deallocation function.

_However_ the entire reason for the existing wording around dropping the first
argument is because for non-type aware deletes there are multiple possible counts
of non-placement args. For type-aware allocation functions, maybe we can actually
just say something about simply comparing the placement-parameters? The would
remove the "ignore 3 from allocator, and 4 from the deallocator" wording?
...
> - [30.4a]{.pnum} [Otherwise, if there is a candidate that is a type-aware deallocation function, candidates that are not type-aware deallocation functions are removed from the set of candidates. The deallocation function is selected by overload resolution using the argument list as specified below and the set of candidates.]{.add}
...
>   - [9.1a.0]{.pnum} [Let S be a set that consists of deallocation functions under consideration.]{.add}
...
>   - [9.1a.2]{.pnum} [Each type-aware deallocation function in _S_ that is a function template is replaced by the function template specializations (if any) generated using template argument deduction ([temp.over], [temp.deduct.call]) with _L_ as the argument list.]{.add}
>   - [9.1a.3]{.pnum} [If any of the deallocation functions in _S_ have the first parameter of the same type as the type-identity argument:]{.add}

[

This came about because given how late sized delete came in i just couldn't believe it
was as broken as it was, so it took me re-reading this repeatedly to actually read
...
so at that point all non-usual-deallocation functions (template functions) have been removed at this point, which i have missed for multiple years. I need to think about this, and talk to louis. 

This may be an EWG issue: the intention has always been to reject this as a hazard reduction, but I think I need to see whether that hazard reduction warrants this very weird interaction.

The intention has always been to reject resolution if there are any type-aware functions that have
a matching type identity, and every variation of the wording, no matter how bad, has attempted to
achieve this. But all of that has _always_ been after paragraph 8. As a result, in the standard 9.1a.3.1 is a no-op: all candidates it could have rejected would already have been removed.

The result is that 

>     - [9.1a.3.1]{.pnum} [Dealloaction functions that are not usual deallocation function or are specializations of function templates that is not usual deallocation functions are removed from _S_.]{.add}

]{.draftnote}
>     - [9.1a.3.2]{.pnum} [Overload resolution shall select the deallocation function using _L_ as the argument list and _S_ as the set of candidates, and the selection process terminates.]{.add}
>
>     [Otherwise, type-aware deallocation functions are eliminated from further consideration.]{.add}
...
## Constant evaluation [expr.const]{.sref .unnumbered}

[do we want to remove this? try to develop a general constexpr new/delete solution?]{.draftnote}
...
** NOTE: This wording changes from **
...
> [basic.stc.dynamic.general]{.sref .unnumbered}
>
> [. . .]
>
> [2b]{.pnum} If there is a type-aware allocation or deallocation function that
> belongs to a class scope S, there shall be an allocation function and a deallocation
> function that belong to S.

:::

** in addition to being generally better, I believe that this wording is actually 
a closer match to the stated semantics **
