...
`__cpp_type_aware_allocators` with a release appropriate value.
...
`template <template<typename> typename> void *operator new(T<int>, std::size_t)`.]{.ednote}
...
> type  `std​::​size_t` ([support.types]{.unnumbered .sref}). [The first]{.rm}[Both the type-identity,
> if present, and the size]{.add} parameters shall not have an associated default argument 
> ([dcl.fct.default]{.unnumbered .sref}). The value of the [first]{.rm}[size]{.add} parameter is
> interpreted as the requested size of the allocation.  An allocation function can be a function
> template. Such a template shall declare its return type and [first]{.rm}[size]{.add} parameter as
> specified above[, and the type-identity parameter shall be a specialization of
> `std::type_identity`]{.add} (that is, template argument types shall not be used in the return type
> and [first]{.rm}[size]{.add} parameter type[, and if present in the declaration of the
> type-identity parameter they only be used in the template argument to `std::type_identity`]{.add}).
...
Due to intended use cases of type aware new and delete it is necessary for type aware operators to
be ignored in consteval environments, there is wording to achieve this, but it needs some work. A
more general fix would be to permit methods to be annotated to prevent visibility in consteval
contexts, butthat would require an additional feature.
]{.ednote}
...
[these points exist, but are being inset and numbered/put into a sub-list]{.ednote}
...
[end the added inset + new sublist]{.ednote}
...
the lack of any mechanism to otherwise force co-definition of new and delete operators]{.ednote}

::: add
> [31]{.pnum} If either the allocation or deallocation function have a type-identity parameter, then
> both functions shall be defined in the same context, otherwise the program is ill formed.
:::
...
[is this sufficient to say that given `X *Obj` for `delete Obj` and `delete [] Obj` will
have a type-identity of type `std::type_identity<X>`?]{.ednote}
...
>    If argument deduction and checking succeeds, the deduced template-arguments are used to
>    synthesize the declaration of a single function template specialization. If the first parameter
>    of the synthesized function is of the same type as the type-identity, it is added to the set of
>    candidates. 
:::

[It is unclear if this last constraint needs to be stated explicitly as it would require a
non-standard declaration of `std::type_identity` which I think would be UB?]{.ednote}
...
>    [over.match.best]{.unnumbered .sref} are removed from the set of candidates.
:::
...
> [9]{.pnum} An implementation may need to allocate additional storage for a coroutine. This storage
> is known as the coroutine state and is obtained by calling a non-array allocation function 
> ([basic.stc.dynamic.allocation]{.unnumbered .sref}). The allocation function's name is looked up
> by searching for it in the scope of the promise type.
>
::: add
>  - [9.1] The type-identity is a default-initialized object of type `std::type_identity<T>` where
>    `T` is the promise type.
:::

>  - [9.1->9.2]{.pnum} If the search finds any declarations, overload resolution is performed on a
>    function call created by assembling an argument list. The first argument [the type-identity,
>    the second]{.add} is the amount of space requested, and is a prvalue of type `std​::​size_t`. The
>    lvalues p[1]{.sub}...p[n]{.sub} are the successive arguments. If no viable function is found
>    ([over.match.viable]{.unnumbered .sref}), overload resolution is performed again on a function
>    call created by passing just the amount of space required as a prvalue of type `std​::​size_t`.
>    [If no viable function is found, this step is repeated without the including the type-identity
>    argument.]{.add}
...
## Design choice: type aware allocation functions are ignored in consteval contexts
...
Our solution is to simply exclude type aware allocation functions from consideration when
resolving new and delete in constant evaluation contexts. We believe that this is a reasonable
trade off, especially as the core problem we face is not new to the feature, just more easily
encountered. We hope the committee addresses this short coming in a more general way that also
covers class scoped operators.
