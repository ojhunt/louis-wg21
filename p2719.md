...
> [1]{.pnum} Objects can be created dynamically during program execution, using _new-expressions_
> ([expr.new]{.sref .unnumbered}), and destroyed using _delete-expressions_ ([expr.delete]{.sref .unnumbered}).
> [An _allocation function_ is a function or function template named `operator new` or `operator new[]`.
> A _deallocation function_ is a function or function template named `operator delete` or `operator 
> delete[]`]{.add}. A C++ implementation provides access to, and management of, dynamic storage
> via the global [_allocation functions_ `operator new` and `operator new[]`]{.rm}[allocation]{.add} and
> [the global _deallocation functions_ `operator delete` and `operator delete[]`]{.rm}[deallocation functions]{.add}.
>
> [2]{.pnum} The library provides default definitions for the global allocation and deallocation
> functions. Some global allocation and deallocation functions are replaceable ([dcl.fct.def.replace]{.sref .unnumbered}).
> The following allocation and deallocation functions ([support.dynamic]{.sref .unnumbered}) are implicitly declared
...
> [2a]{.pnum} The first parameter _P_ of an allocation or deallocation function is the _type-identity parameter_ if the type _U_ of _P_ is a specialization of `std::type_identity` ([meta.trans.other]{.sref .unnumbered}) and, if _U_ is dependent, _U_ has the shape `TT<T>` for some type `T`, where `TT` designates `std::type_identity` and is not dependent. An allocation function that has a type-identity parameter is a _type-aware allocation function_. A deallocation function that has a type-identity parameter is a _type-aware deallocation function_.

> [A parameter with a dependent type that names std::type_identity via an alias template cannot be a type-identity parameter.]{.note}

> [2b]{.pnum} If there is a type-aware allocation or deallocation function that belongs to a class scope _S_, there shall be **both (is this needed?)** an allocation function and a deallocation function that belong to _S_.

:::

> [3]{.pnum} If the behavior of an allocation or deallocation function does not 
> satisfy the semantic constraints specified in [basic.stc.dynamic.allocation]{.sref .unnumbered} and
> [basic.stc.dynamic.deallocation]{.sref .unnumbered}, the behavior is undefined.
...
[I'm unsure of the semantic impact of the change p1? (for allocation or deallocation)]{.draftnote}
...
> [1a]{.pnum} The [declared]{.add} return type [of an allocation function _F_]{.add} shall be [the non-dependent type]{.add} "pointer to `void`". [The minimum number of parameters of _F_ is determined
as follows:]{.add}

::: add

>   [1a.1]{.pnum}
>      - If _F_ is a type-aware allocation function, then _F_ shall have at least three
>        parameters, where the second parameter is called the _size parameter_ and the third parameter
>        is called the _alignment parameter_.
>
>   [1a.2]{.pnum}
>      - Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters,
>        where the first parameter is called the _size parameter_.
>
>   [1a.3]{.pnum}
>      - Otherwise, F shall be a function that has at least one parameter, where the first parameter
>        is called the _size parameter_.
>
> The following restrictions apply to the parameters of _F_:
>
>   [1a.4]{.pnum}
>      - The type-identity parameter shall not have a default argument ([dcl.fct.default]{.unnumbered .sref}).

:::

>   [1a.5]{.pnum}
>     - The [first]{.rm}[size]{.add} parameter shall have [the non-dependent]{.add}
>       type `std​::​size_t` ([support.types]{.unnumbered .sref}) and shall not have a default argument.

::: add

>   [1a.6]{.pnum} 
>     - The alignment parameter shall have the non-dependent type `std::align_val_t`
>       ([support.dynamic]{.unnumbered .sref}) and shall not have a default argument.

:::

deallocation of an object of type U:
let candidates = resolved functions with named operator delete/operator delete[]

if (candidates contains any type aware deallocation functions) {
  for (candidate_template: candidates.filter([](c) {return c is a function template} ) {
    let L = {type_identity<U>{}, (void*){}, (std::size_t){}, (std::align_val_t){} }'
    let specialized_candidate = try_deduction(candidate_template, L)
    if (!specialized_candidate)
      contine;
    // specialized_candidate is a deallocation function of type
    //   void(type_identity<U>, void*, size_t, align_val_t)
    candidates.push_back(specialized_candidate)
  }
  // candidates now contains
  // zero or more non-type-aware deallocation functions
  // zero or more type-aware functions that are not template specializations in which the first parameter is not type_identity<U>
  // zero or more type-aware deallocation functions of type void(type_identity<U>, void*, std::size_t, std::align_val_t)
  // zero or more type-aware deallocation functions of type void(type_identity<U>, void*, std::size_t, std::align_val_t, ...)
  // zero or more type-aware deallocation functions of type void(type_identity<U>, void*, std::size_t, std::align_val_t, placement_parameters...)

  let potential_candidates = candidates.filter([](C){C.parameters(0).type == type_identity<U>})
  // at this point potential_condidates contains only:
  //   zero or more functions of type void(type_identity<U>, void*, std::size_t, std::align_val_t)

  if (!potential_candidates.empty()) {
    potential_candidates.remove([](C){
      if (C is a vararg function)
        return true; // remove candidates that are vararg functions
      if (C is a specialized tempate function, and the sepecialized template had variadic template parameter)
        return true; // remove candidates specialized from variadic function templates
      if (C.paramCount() != 4)
        return true; // remove candidates that had parameters with assigned default arguments
    })

    // at this point we have zero or more potential candidates of type void(type_identity<U>, void*, std::size_t, std::align_val_t)
    if (potential_candidates.empty()) {
      // We had found candidates that were trying to operate over the type U, but none of them are valid
      error()
    }

    // at this point we have at least one function where the first parameter is of type type_identity<U>
    let resolved_candidate = overload_resolution(potential_candidates, L)
    if (!resolved_candidate) { // given all our constraints have we already actually reduced the candidate list down to one by definition?
      // we had some functions where the type_identity was correct, but none matched, which is an error
      error()
    }
    // at this point selected_candidate is a function of type void(type_identity<U>, void*, std::size_t, std::align_val_t), that we can call with arguments of the same type as L
    selected_function = resolved_candidate
    goto finised_selection;
  }
  candidates.remove([](C) {return C is a type-aware deallocation function})
  // candidates now contains only non-type-aware deallocation functions
}
// at this point candidates no longer contains any type aware allocation functions

selected_function = the result of the non-type-aware selection process, or we've failed with an error

finished_selection:
// at this pointer selected_functions is either
//   at type-aware deallocation function of type 
make the call, etc

::: draftnote

NOTE while editing: a non-type-aware allocation (or deallocation) function places _no_ constraints on anything other than the return type and size parameter. The _alignment_ parameter for such functions is not specified: the aligned allocation is performed in expr.new by saying essentially:

`new (optional_placement_args...) SomeOveralignedType`:

1. resolved_scope = walk up the scope hierarchy of the class `SomeOveralignedType` to find a scope containing a function named operator new, or use the global scope
2. attempt to call 

```cpp
resolved_scope::operator new(sizeof(SomeOveralignedType), std::align_val_t(alignof(SomeOveralignedType)), optional_placement_args...)
```

3. if that works you're done, otherwise try 
```cpp
resolved_scope::operator new(sizeof(SomeOveralignedType), optional_placement_args...)
```

The effect of this is you can do

```cpp
new (std::align_val_t(arbitrary_alignment)) SomeObject;
```

if SomeObject is overaligned we first try to call an "aligned" operator new, resulting in

```cpp
operator new(sizeof(SomeObject), std::align_val_t(alignof(SomeObject), std::align_val_t(std::align_val_t(arbitrary_alignment))))
```

which fails, so we fall back to an "unaligned" allocation (e.g. we just call operator new(size, placement args...)), meaning we try:

```cpp
operator new(sizeof(SomeObject), std::align_val_t(std::align_val_t(arbitrary_alignment))))
```

which succeeds and we end up calling the aligned allocator, if `SomeObject` is not overaligned, so we don't include the alignment, which means the first attempt works, and again calls the aligned allocator.

it also means:

```cpp
template <class T> void *operator new(size_t, T)
// or
template <class... Ts> void *operator new(size_t, Ts...)
```

will also match the attempt to call an aligned allocator. 🤷‍♂️

The above template cases exist in the real world, and keeps on surprising the authors of said code, despite us repeatedly explaining the problem.

:::

::: rm

> The first parameter shall not have an associated default argument ([dcl.fct.default]{.unnumbered .sref}).
> The value of the first parameter is interpreted as the requested size of the allocation. An allocation
> function can be a function template. Such a template shall declare its return type and first parameter
> as specified above (that is, template parameter types shall not be used in the return type and first
> parameter type). Allocation function templates shall have two or more parameters.
...
> The template argument of the type-identity parameter is interpreted as the **unqualified?** type of
> the complete object that will use the allocation as its storage. An allocation function can have
> additional parameters that are not specified in this document.
:::
:::

::: draftnote

The new wording for the return type of allocation and deallocation functions might resolve [@CWG1676] "auto return type for allocation and deallocation functions", as it follows the approach in [@CWG1669] "auto return type for main"
...
> [1]{.pnum} A deallocation function [that is not a class member function]{.rm}shall [either be a class member]{.add} or belong to the global scope and not have a name with internal linkage.

::: add

> [1a]{.pnum}  The declared return type of a deallocation function _F_ shall be the non-dependent type `void`. The following restrictions apply to the parameters of _F_:

> - [1a.1]{.pnum} If _F_ is a type-aware deallocation function, then _F_ shall have at least four parameters, where
>    - [1a.1.1]{.pnum} the second parameter is called the _address parameter_,
>    - [1a.1.2]{.pnum} the third parameter shall have the non-dependent type `std::size_t` ([support.types]), and
>    - [1a.1.3]{.pnum} the fourth parameter shall have the non-dependent type `std::align_val_t` ([support.dynamic]).
> - [1a.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is called the _address parameter_.
> - [1a.3]{.pnum} Otherwise, _F_ shall be a function that has at least one parameter, where the first parameter is called the _address parameter_.
...
> [1b]{.pnum} A _usual deallocation function_ is [a type-aware deallocation function with four parameters, or]{.add} a deallocation function whose parameters after the [first](address){.sub} parameter [are]{.add}

::: add
> - [1b.1]{.pnum} optionally, a parameter of type `std::destroying_delete_t` ([new.syn]{.sref .unnumbered}), then
> - [1b.2]{.pnum} optionally, a parameter of type `std::size_t` **19**, then
> - [1b.3]{.pnum} optionally, a parameter of type `std::align_val_t`.
:::

> [2]{.pnum} A [usual]{.add} deallocation function [named `operator delete`]{.add} is a _destroying operator delete_ if it has at least two parameters [and its first parameter is of type "pointer to `C`", where `C` is a class type]{.add}, and its second parameter is of type `std::destroying_delete_t`. [A destroying operator delete shall be a class member function named operator delete.](A destroying operator delete shall belong to a class scope introduced by `C`.){.sub}

> [Array deletion cannot use a destroying operator delete.]{.note}
...
> [19]{.pnum} The `new-placement` syntax is used to supply additional arguments to an allocation function; such an expression is called a _placement_ `new-expression`.

::: add

> [19a]{.pnum} The _type-identity argument_ is a value of type `std::type_identity<U>` ([meta.trans.other]{.unnumbered .self}), where `U` is a cv-unqualified version of the array element type if the allocated type is an array type, and a cv-unqualified version of the allocated type otherwise.
:::
...
> - [20.1]{.pnum} _T_ be the type-identity argument,

> - [20.2]{.pnum} _S_ be an expression of type `std::size_t` equal to the amount of space requested,

> - [20.3]{.pnum} _A_ be an expression of type `std::align_val_t` equal to the alignment of the allocated type, and

> - [20.4]{.pnum} _P_ be the `initializer-clause`s in the expression-list if the `new-placement` syntax is used, and an empty set otherwise.
...
> - [20.5]{.pnum} (_T_, _S_, _A_, _P_) if _P_ is an empty set or the first initializer-clause in _P_ does not have the type `std::align_val_t` after parameter transformations ([dcl.fct]), and (_T_, _S_, _P_) otherwise, or, if overload resolution fails,

:::

::: draftnote
The wording of 20.5 is intented to support the 
```cpp
new (std::align_val_t(custom_alignment)) T
```
idiom for forcing over-aligned allocation.
:::

::: add

> - [20.6]{.pnum} (_S_, _A_, _P_) if the type of the allocated object has new-extended alignment, and (_S_, _P_) otherwise, or, if overload resolution fails,

> - [20.7]{.pnum} (_S_, _P_) if the type of the allocated object does not have a new-extended alignment, and (_S_, _A_, _P_) otherwise.
...
> For a non-placement allocation function, the normal deallocation function lookup is used to find the matching deallocation function ([expr.delete]). For a placement allocation function, the selection process is described below. In any case, the matching deallocation function (if any) shall be non-deleted and accessible from the point where the new-expression appears.
...
> - [30.4a]{.pnum} Otherwise, if there is a candidate that is a type-aware deallocation function, candidates that are not type-aware deallocation functions are removed from the set of candidates. There shall be the best viable function ([over.match.best]) in the set of candidates, that function is selected.
...
TODO: update the argument list 

## [expr.delete]{.sref .unnumbered}
::: add
> [8a]{.pnum} Given an operand of type "pointer to _cv_ `U`", the _type-identity argument_
> is a value of type `std::type_identity<U>` ([meta.trans.other]).
:::

::: draftnote
 p9 below is trying to update the existing overload resolution semantics to say
...
<!-- :::add
>
> [9]{.pnum} For each function template found, function template argument deduction
...
>
>  - [9.1]{.pnum} If any of the deallocation functions is a destroying operator delete, all
>    deallocation functions that are not destroying operator deletes are eliminated from further
>    consideration.

::: add
>  - [9.1a]{.pnum} If any of the deallocation functions are a type-aware deallocation function, all functions that are not type-aware deallocation functions are removed from further consideration, then
>
>    - [9.1a.0]{.pnum} pull expr.new 30 template instantiation
>        - if BVF 
>    - [9.1a.1]{.pnum} Each candidate that is a function template is replaced by the function template specializations (if any) generated using template argument deduction ([temp.over]{.unnumbered .sref}, [temp.deduct.call]{.unnumbered .sref}) with the arguments
>      - the type-identity argument
>      - an unspecified value of type "pointer to `void`"
>      - an unspecified value of type `std::size_t`
>      - an unspecified value of type `std::align_val_t`
>    - [9.1a.2]{.pnum} The most viable function [over.match.best]{.unnumbered .sref} shall be a type-aware deallocation function with a type-identity parameter of type `std::type_identity<U>`.
>    - [9.1a.3]{.pnum} That function is selected and the selection process terminates.
>
>    [Is this sufficient to ensure that if there is no "most viable function", or the type-identity argument does not have the correct type, this reduces to a diagnosed error, or does this need to be handled explicit]{.draftnote}
:::

>  - [9.2]{.pnum} If the type has new-extended alignment, a function with a parameter of type
...
> [. . .]
>
> [10]{.pnum} For a single-object delete expression, the deleted object is the object `A` pointed
...
> stated above.]{.note} [where is it stated above? not sure right now due to changes and memory]{.draftnote}
> [If the selected function is a type-aware deallocation function, then it will be called with the
> type-identity argument as its first argument]{.add}. For an array delete expression,
...
> overhead ([expr.new]) in an array delete expression, as [its first argument](the address argument){.sub}.
>
> ::: note
...
I don't believe that the overload resolution actually constrains where the `std::align_val_t` or `std::size_t` parameters are in the candidate function signature. It also seems to just specify a call, which would imply default parameters are permitted. On the other hand i'm not sure if its specific way of constructing the call bypasses default parameter configuration

If as specified default parameters are supported, then
```cpp
void operator delete(void*, float*=0, std::align_val_t = {})
```

could match the requirements for an aligned delete?

Similarly the lack of actual positioning information seems to imply that these are both valid candidates
