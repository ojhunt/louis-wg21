...
> - [30.4a]{.pnum} [Otherwise, if there is a candidate that is a type-aware deallocation function, candidates that are not type-aware deallocation functions are removed from the set of candidates. The deallocation function is selected by overload resolution using the argument list as specified below and the set of candidates.]{.add} [I _think_ this is unnecessary with the addition of p30.1a.1]{.draftnote}
