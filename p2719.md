...
> _type-identity parameter_, and the second parameter is called the _size parameter_. Otherwise, the
> first parameter is called the size parameter.]{.add} The [first]{.rm}[size]{.add} parameter shall
> have type `std​::​size_t` ([support.types]{.unnumbered .sref}). [The first]{.rm}[Neither the
> type-identity, if present, nor the size]{.add} parameters shall [not ]{.rm}have an associated
> default argument  ([dcl.fct.default]{.unnumbered .sref}). The value of the
> [first]{.rm}[size]{.add} parameter is interpreted as the requested size of the allocation. An
> allocation function can be a function template. Such a template shall declare its return type and
> [first]{.rm}[size]{.add} parameter as specified above[, and the type-identity parameter shall be
> a specialization of `std::type_identity`]{.add} (that is, template [arguments]{.add}[argument
> types]{.rm} shall not be used in the return type and [first]{.rm}[size]{.add} parameter type[,
> and if present in the declaration of the type-identity parameter, they shall only be used in the
> template argument to `std::type_identity`]{.add}). [If such a function or
> function template does not have a type-identity parameter it]{.add} shall have two or more
> parameters.
...
instantiation]{.draftnote-for-cwg}
...
> is called the object parameter.]{.add}
> If the function is a destroying operator delete declared in class type C, the type of its
> [first]{.rm}[address]{.add} parameter shall be `C*`; otherwise, the type of its
> [first]{.rm}[address]{.add} parameter shall be `void*`. A deallocation function may have [more
> than one parameter]{.rm}[additional parameters beyond the type-identity and address
> parameters]{.add}. []{.add}
...
> [can be a function template. Such a template shall declare its address parameter as specified
> above. If present the type-identity parameter shall be a specialization of `std::type_identity`
> (that is, template arguments shall not be used in the address parameter type, and if present in
> the declaration of the type-identity parameter, they shall only be used in the template argument
> to `std::type_identity`). If such a function or function template does not have a type-identity
> parameter it shall have two or more parameters.]{.add}
> A template instance [is never a usual deallocation function,
> regardless of its signature.]{.rm}[is only a usual deallocation function if it has at least two
> parameters, the first parameter is a type-identity, and the type-identity
> is the only dependent parameter (i.e. the type-identity parameter may be of type
> `std::type_identity<T>` where `T` is a dependent type).]{.add}
...
::: add
> [20]{.pnum} The _type-identity argument_ is a value of type `std::type_identity<U>`.
> When the allocated type is “array of N T”, U is T with qualifiers removed; otherwise, U is the
> allocated type with qualifiers removed.
:::
...
>  - [+ 21.1]{.pnum} [The first argument is the type-identity argument, the second]{.add}[The first]{.rm}
...
the lack of any mechanism to otherwise force co-definition of new and delete operators]{.draftnote
audience=CWG&EWG}
...
## expr.delete
::: add
> [9]{.pnum} For an delete operator applied to an operand of type `T*`, _type-identity argument_ is a
> value of type `std::type_identity<U>` where `U` is the result of removing all qualifiers from the
> type `T`.
:::
...
non-standard declaration of `std::type_identity` which I think would be UB?]{.draftnote question=CWG}
...
Alternative approachs, that might also be useful for other contexts (static analysis, debug tooling,
etc) would be:
...
We'd like to acknowledge the time spent by Jens Meurer, Brian Bi, and Corentin to help us develop
