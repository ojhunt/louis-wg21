...
>
> [1a]{.pnum} [The declared return type of a deallocation function _F_ shall be the non-dependent type `void`.]{.add}
> [The following restrictions apply to the parameters of _F_:]{.add}
>
> - [[1a.1]{.pnum} If _F_ is a type-aware deallocation function, then _F_ shall have at least four parameters, where]{.add}
>   - [[1a.1.1]{.pnum} the second parameter is called the _address parameter_,]{.add}
>   - [[1a.1.2]{.pnum} the third parameter shall have the non-dependent type `std::size_t` ([support.types]), and]{.add}
>   - [[1a.1.3]{.pnum} the fourth parameter shall have the non-dependent type `std::align_val_t` ([support.dynamic]).]{.add}
> - [[1a.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is called the _size parameter_.]{.add}
> - [[1a.3]{.pnum} Otherwise, _F_ shall be a function that has at least one parameter, where the first parameter is called the _address parameter_.]{.add}
>
...
> :::
>
> [1b]{.pnum} A _usual deallocation function_ is [a type-aware deallocation function with four parameters, or]{.add} a deallocation function whose parameters after the [first]{.rm} [address parameter]{.add}
>
>   - [[1b.1]{.pnum} optionally, a parameter of type `std::destroying_delete_t`, then]{.add}
>   - [[1b.2]{.pnum} optionally, a parameter of type `std::size_t`^19^, then]{.add}
>   - [[1b.3]{.pnum} optionally, a parameter of type `std::align_val_t`.]{.add}
>
> [2]{.pnum} A [usual]{.add} deallocation function is a _destroying operator delete_ if [it has at least two parameters]{.rm} [its first parameter is of type "pointer to `C`", where `C` is a class type,]{.add} and its second parameter is of type `std::destroying_delete_t`. [A destroying operator delete shall be a class member function named `operator delete`.]{.rm} [A destroying operator delete shall belong to a class scope introduced by `C`.]{.add}
>
> ::: note
...
>
> ::: rm
> [3]{.pnum} Each deallocation function shall return `void`.
> If the function is a destroying operator delete declared in class type `C`, the type of its first parameter shall be "pointer to `C`"; otherwise, the type of its first parameter shall be "pointer to `void`".
> A deallocation function may have more than one parameter.
> A _usual deallocation function_ is a deallocation function whose parameters after the first are
>
...
>
> [19a]{.pnum} [The _type-identity argument_ is a value of type `std::type_identity<U>` ([meta.trans.other]), where `U` is a cv-unqualified version of the array element type if the allocated type is an array type, and a cv-unqualified version of the allocated type otherwise.]{.add}
>
...
> [Let]{.add}
>
> - [20.1]{.pnum} [_T_ be the type-identity argument,]{.add}
...
>     - [9.1a.1.1]{.pnum} [a value of type `std::type_identity<U>` ([meta.trans.other]),]{.add}
...
>   - [9.1a.3]{.pnum} [If any of the deallocation functions have the first parameter of the type `std::type_identity<U>`, then overload resolution shall select the deallocation function using _L_ as the argument list and deallocation functions as the set of candidates, and the selection process terminates. Otherwise, type-aware deallocation functions are eliminated from further consideration.]{.add}
