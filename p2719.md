...
`template <template<typename> typename> void *operator new(T<int>, std::size_t)`.]{.question group=CWG}
...
instantiation]{.question-for-cwg}
...
[these points exist, but are being inset and numbered/put into a sub-list]{.review_note}
...
[end the added inset + new sublist]{.review_note}
...
the lack of any mechanism to otherwise force co-definition of new and delete operators]{.question group=CWG}
...
have a type-identity of type `std::type_identity<X>`?]{.question group=CWG}
...
non-standard declaration of `std::type_identity` which I think would be UB?]{.question group=CWG}
...
type being allocated. For P2719 we are leaving coroutine allocations as unsupported, however the
coroutine specification will need to be updated in future to provide type information to allocators.
