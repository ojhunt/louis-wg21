...
- R6 (EWG and CWG Brno26):
  ...
  - Updated wording to address changes introduced in the current specification,
    and the additional improvements introduced by P3769
  - Rewrite the wording entirely to be more in line with core 
  - Added design choice on the behavior of delete when no usual type-aware delete
    functions are declared but there are non-usual placement type-aware deallocation.
...
0. Annoy Hana and Erich- try to steal time on monday
  a) constexpr? -> defer to feedback from core
  b) alias templates ???
1. Ensure wording is precise around type_identity, note it prevents alias templates -> needs to go to EWG
6. Look at all changes, verify sufficient context
...
[The wording here is intended to support the declaration of `operator new`, `operator new[]`, `operator delete`, or `operator delete[]` in which it is known, at the point of the function declaration, that first parameter is a specialization of `std::type_identity` - e.g. verifying that it is a specialization of type_identity occurs prior to specialization of a dependent declaration]{.draftnote}
...
> - [[1a.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is called the _size parameter_ and the second parameter is called the _alignment parameter_.]{.add}
...
> [1]{.pnum} A deallocation function [that is not a clas member function]{.rm} shall [either be a class member or]{.add} belong to the global scope and not have a name with internal linkage.
>
...
> - [[1a.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is called the _address parameter_.]{.add}
...
> - [20.4]{.pnum} [_P_ be the #initializer-clause#s in the #expresion-list# of the #new-placement# if the #new-placement# syntax is used, and an empty set otherwise.]{.add}
>
...
::: draftnote
The intent of the test for a placement argument of type `std::align_val_t`
paragraph 20.6 is to support the idiom in which a type that is not overaligned
is allocated with extended alignment:

```cpp
new (std::align_val_t(some_alignment)) T;
```

For non-type-aware allocation functions this works because the way we resolve
the correct operator new for such a type is to construct an argument list of
`{sizeof(T), placement-arguments...}`, then perform overload resolution to find
a matching function. With this idiom the result is that the argument list is
`{sizeof(T), std::align_val_t(some_alignment)}` which results in overload
resolution matching `operator new(std::size_t, std::align_val_t)`, e.g the
aligned allocation function.

In a type-aware allocation function, the alignment parameter is mandatory, so
absent this test using this idiom would result in an argument list consisting
of `{sizeof(T), alignof(T), std::align_val_t(some_alignment)}`. Overload
resolution would not resolve this to a type aware allocation function unless
such a function was written as

```cpp
void *operator new(type-identity-parameter, std::size_t, std::align_val_t, std::align_val_t);
```

which would be both confusing to a developer, and counter the goal of the idiom
which is to simply force the standard aligned argument to be a custom value. By specialising the construction of the argument list on the type of the first
placement parameter (if there are placement parameters), the developer's goal
is achieved as the argument list becomes `{type-identity-argument, sizeof(T), std::align_val_t(some_alignment)}`, which then resolves to a non-placement
type-aware allocation function.
:::
>
>
> ::: example
...
> - [30.1a]{.pnum} [If the selected allocation function is a type-aware allocation function]{.add}
>   - [30.1a.1]{.pnum} [Each candidate that is a not a type-aware deallocation function whose a type-identity parameter that is the same type as the the type-identity parameter of the allocation function is removed from the set of candidates.]{.add}
>   - [30.1a.2]{.pnum} [Each candidate whose parameter-type-list is not identical to that of the allocation function, after their respective alignment parameters, is removed from the set of candidates.]{.add}
> - [30.2]{.pnum} [Otherwise, each ]{.add}[Each]{.rm} candidate whose parameter-type-list is not identical to that of the allocation function, ignoring their respective first parameters, is removed from the set of candidates.
>

::: draftnote

The original p30.2 is insufficient on its own for type-aware allocation and deallocation functions:

```cpp

struct S {
  S() { throw ""; }
  void *operator new(std::type_identity<S>, std::size_t, std::align_val_t, float); // #1
  void  operator delete(std::type_identity<S>, void *, std::size_t, std::align_val_t, float); // #2
};
...
new S;
```

The parameter-type-list for the #1 is:
```cpp
{std::type_identity<S>, std::size_t, std::align_val_t, float}
```

and for what _should_ be the matching operator delete:

```cpp
{std::type_identity<S>, void *, std::size_t, std::align_val_t, float}
```

skipping the first parameter we get

```cpp
{std::size_t, std::align_val_t, float}
```

and

```cpp
{void *, std::size_t, std::align_val_t, float}
```

which don't match and would therefore be excluded

:::

> - [30.3]{.pnum} Each candidate whose associated constraints (if any) are not satisfied ([temp.constr.constr]) is removed from the set of candidates.
> - [30.4]{.pnum} If exactly one function remains, that function is selected.
> - [30.4a]{.pnum} [Otherwise, if there is a candidate that is a type-aware deallocation function, candidates that are not type-aware deallocation functions are removed from the set of candidates. The deallocation function is selected by overload resolution using the argument list as specified below and the set of candidates.]{.rm} [I _think_ this is unnecessary with the addition of p30.1a.1]{.draftnote}
...
> :::
>
> [31a]{.pnum} If a #new-expression# calls a type-aware deallocation function, it passes the type-identity argument as the first argument, and the value returned from the allocation function call as the second argument, of type "pointer to `void`".
>
> [32]{.pnum} [If]{.rm}[Otherwise, if]{.add} a #new-expression# calls a deallocation function, it passes the value returned from the allocation function call as the first argument of type [`void*`]{.rm}["pointer to `void`"]{.add}.
...
>
> - [8.1]{.pnum} For a single-object delete expression, if the operand is a pointer to cv T and T has a virtual destructor, the deallocation function is the one selected at the point of definition of the dynamic type's virtual destructor ([class.dtor]).
> - [8.2]{.pnum} Otherwise, a search is performed for the deallocation function's name in the scope of `T`.
>
> Otherwise, or if nothing is found, the deallocation function's name is looked up by searching for it in the global scope. [In any case, any declarations other than of usual deallocation functions ([basic.stc.dynamic.deallocation]) are discarded.]{.rm}
>
> [8.2a]{.pnum} [Any declarations that are not usual deallocation functions ([basic.stc.dynamic.deallocation]) are discarded.]{.add}
>
> ::: note
...
> :::
>
> [8a]{.pnum} [Given an operand of type “pointer to _cv_ `U`”, the type-identity argument is a value of type `std::type_identity<U>` ([meta.trans.other]).]{.add}
>
...
>   - [9.1a.0]{.pnum} [Let S be a set that consists of deallocation functions under consideration.]{.rm}
>   - [9.1a.1]{.pnum} [Let _L_ be a list of arguments for a function call that consists of]{.add}
>     - [9.1a.1.1]{.pnum} [the type-identity argument,]{.add}
...
>   - [9.1a.2]{.pnum} [Each [type-aware]{.rm} deallocation function [in _S_]{.rm} that is a function template is replaced by the function template specializations (if any) generated using template argument deduction ([temp.over], [temp.deduct.call]) with _L_ as the argument list.]{.add}
>   - [9.1a.3]{.pnum} [If any of the deallocation functions [in _S_]{.rm} have the first parameter of the same type as the type-identity argument, then overload resolution shall select the deallocation function using _L_ as the argument list and [_S_]{.rm} deallocation functions as the set of candidates, and the selection process terminates. Otherwise, type-aware deallocation functions are eliminated from further consideration.]{.add}
...
> [If the selected function is a type-aware deallocation function, the type-identity argument is passed as the argument correponding to the type-identity parameter.]{.add}
...
## Feature detection [cpp.predefined]{.sref .unnumbered}
