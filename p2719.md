...
> member function shall belong to the global scope and not have a name with internal linkage.
...
** NOTE: there is no constraint or semantic meaning assigned to the second parameter of a frunction template, it just has to exist because, um, reasons. **

***

** NOTE FOR OLIVER: look at 1a.2 a million billion times because it's still terrible **

> [1a]{.pnum} The [declared]{.add} return type [of an allocation function _F_]{.add} shall be [the
> non-dependent type]{.add} "pointer to `void`". [The minimum number of parameters of _F_ is determined
> as follows:]{.add}
...
> [1a.1]{.pnum} -- If _F_ is a type-aware allocation function, then _F_ shall have at least three
...
>
> [1a.2]{.pnum} -- Otherwise, if _F_ is a function template it shall have at least two parameters,
> or it shall be a function with at least one parameter, where the first parameter is called
> the _size_ parameter.
>
> The following restrictions apply to the parameters of _F_:
>
> [1a.3]{.pnum} -- The type-identity parameter shall not have a default argument
> ([dcl.fct.default]{.unnumbered .sref}).

:::

> [1a.4]{.pnum} -- The [first]{.rm}[size]{.add} parameter shall have [the non-dependent]{.add} type `std​::​size_t`
> ([support.types]{.unnumbered .sref}) and shall not have a default argument.

::: add

> [1a.5]{.pnum} -- The alignment parameter shall have the non-dependent type `std::align_val_t`
...
> The [declared]{.add} return type shall be
> [the non-dependent type]{.add} "pointer to `void`". [A non-template allocation function shall have at least
...
> `std::align_val_t`]{.add}. [The first parameter shall not]{.rm} [The size parameter,
> and the type-identity and alignment parameters of a type-aware allocation function,
> shall not]{.add} have an associated default argument[s]{.add} ([dcl.fct.default]{.unnumbered .sref}). 

> [The type-identity parameter represents the type being allocated, such that
> when allocating an object or array objects of type `cv U`, the type-identity
> parameter shall be of type `std::type_identity<U>`.]{.add} The value of
...
> used in the template argument to `std::type_identity` when referenced in the type-identity parameter
> of a type-aware allocation function)]{.add}.

:::

::: draftnote

The new wording for the return type of allocation and deallocation functions might resolve [CWG1676]{.ref} "auto return type for allocation and deallocation functions", as it follows the approach in [CWG1669] "auto return type for main"
...
> [1]{.pnum} A deallocation function that is not a class member function shall either be a class member or belong to the global scope and not have a name with internal linkage.

::: add

> [1a]{.pnum}  The declared return type of a deallocation function F shall be the non-dependent type void. The following restrictions apply to the parameters of F:

> [1a.1]{.pnum} -- If F is a type-aware deallocation function, then F shall have at least four parameters, where

> [1a.1.1]{.pnum} -- the second parameter is called the address parameter,
...
** TODO: bring in line with allocation, or seemingly the reverse **


> [1a.2]{.pnum} --  Otherwise, if F is a function template, then F shall have at least two parameters, where the first parameter is called the address parameter.

> [1a.3]{.pnum} -- Otherwise, F shall be a function that has at least one parameter, where the first parameter is called the address parameter.
...
> [1b]{.pnum} A _usual deallocation function_ is [a type-aware deallocation function with four parameters, or]{.add} a deallocation function whose parameters after the [first](address){.sub} parameter

::: add
> [1b.1]{.pnum} -- optionally, a parameter of type `std::destroying_delete_t`, then

> [1b.2]{.pnum} -- optionally, a parameter of type `std::size_t`, then

> [1b.3]{.pnum} -- optionally, a parameter of type `std::align_val_t`.
:::

> [2]{.pnum} A [usual]{.add} deallocation function is a _destroying operator delete_ if [it has at least two parameters](its first parameter is of type "pointer to `C`", where `C` is a class type,){.sub}, and its second parameter is of type `std::destroying_delete_t`. [A destroying operator delete shall be a class member function named operator delete.](A destroying operator delete shall belong to a class scope introduced by `C`.){.sub}

> [Array deletion cannot use a destroying operator delete.]{.note}

::: add
> [A destroying operator delete cannot be a function template, because it cannot have a type-identity parameter]{.note}
:::
...
(3.2) optionally, a parameter of type std::size_t19, then
...
3a If a deallocation function with a type-identity parameter P belongs to a scope S which is not the global scope, there shall be an allocation function with a type-identity parameter of the same type as the type of P that belongs to S.


> [3]{.pnum} A deallocation function or function template shall return `void`. [A deallocation
> function has at least one parameter. If the first parameter is of type `std::type_identity<U>`
> for some type `U` it is called a _type-aware_ deallocation function, and there shall be at least four parameters;
> the first parameter is called the _type-identity parameter_, the second parameter is
> called the _address parameter_, the third parameter is called the _size parameter_,
> and the fourth parameter is called the _alignment parameter_. Otherwise,
> the first parameter is called the _address parameter_. If the function has a type-identity parameter,
> it is not a destroying operator delete.]{.add} If the function is a destroying operator delete declared in
> class type C, the type of its [first]{.rm}[address]{.add} parameter shall be "pointer to `C`"; otherwise, the
> type of its [first]{.rm}[address]{.add} parameter shall be "pointer to `void`". [If the function has a
...
This does not need to explicitly call out type-aware deallocation here: the same
semantics fall out, but it seemed more clear to explicitly state the four parameter
type aware deallocation function than rely on the "optionally, .." capturing the
mandatory parameters for type-aware deallocation.

:::

> A _usual deallocation function_ is [a four parameter type-aware deallocation function,
> or]{.add} a deallocation function whose parameters after the [first]{.rm}[address
> parameter]{.add} are
>
...
> If a type aware deallocation function with placement parameters is declared in
> a class scope, a type aware allocation function accepting a type-identity
> parameter of the same type shall also be declared in the same scope. If the
> type-identity parameter is a dependent type it is ill-formed with no diagnostic
...
The wording of 21.1 supports the 
```cpp
new (std::align_val_t(custom_value)) T
```
...
> [30.4]{.pnum} -- If multiple matching type-aware deallocation functions are
> found with, all candidate functions other than the most viable
> type-aware candidate [over.match.best]{.unnumbered .sref}, are removed from
> the set of candidates. If there are multiple ambiguous type-aware candidates,
> the program is ill formed.

> [30.4]{.pnum} -- If multiple matching deallocation functions with type-identity
> parameters are found, there shall be the best viable function among them ([over.match best]).

>
> [30.5]{.pnum} -- If a single matching deallocation function is found and either the
> allocation function or the deallocation function has a type-identity
> parameter, then both functions shall belong to the same scope 
> ([basic.scope.scope]{.sref .unnumbered}), otherwise the program is ill-formed.

:::

> [30.6]{.pnum} -- Otherwise, no deallocation function is selected.

## [expr.delete]{.sref .unnumbered}
::: add
> [9]{.pnum} Given an operand of type "pointer to `cv U`", the _type-identity argument_
...
::: add
>  - [11.4]{.pnum} If multiple functions of the same type are found, only the most viable function
>    [over.match.best]{.unnumbered .sref} is retained, all others are eliminated from further
>    consideration.
:::
